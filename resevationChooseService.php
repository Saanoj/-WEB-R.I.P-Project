            ...
            <div class="container">
              ...
              while($unService = $services->fetch())
              {
                ...
                if(!in_array($unService["categorie"],$categorieSet)){
                  $req = $bdd->getPDO()->prepare('SELECT idService FROM services WHERE categorie = :categorie');
                  $req->bindValue(':categorie', $unService["categorie"]);
                  $req->execute();
                  $servicesInSameCategorie = $req->fetchAll();
                  array_push($categorieSet,$unService["categorie"]);
                  ?>
                  <?php //foreach ($servicesInSameCategorie as $key => $value) { echo "categorie".$value["idService"]." "; }?>
                  <li class="list-group-item row">
                    <a class="display-4"><?php echo $unService["categorie"] ?></a>
                    <button class="btn btn-primary" type="button" data-toggle="collapse" data-target=".multi-collapse" aria-expanded="false" aria-controls="categorie11 categorie12 categorie13<?php //foreach ($servicesInSameCategorie as $key => $value) { echo "categorie".$value["idService"]." "; }?>">Toggle</button>
                  </li>
                  <?php
                  //var_dump($servicesInSameCategorie);
                }
                ?>
                <li collapse="isCollapsed" class="collapse multi-collapse list-group-item row" id="categorie<?php echo $unService["idService"] ?>">
