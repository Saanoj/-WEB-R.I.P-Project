<?php
require_once __DIR__ . '/Class/Database.php';
require_once __DIR__ . '/Class/Abonnement.php';
require_once __DIR__ . '/Class/BilletTourisme.php';
require_once __DIR__ . '/Class/Chambre.php';
require_once __DIR__ . '/Class/Collaborateur.php';
require_once __DIR__ . '/Class/Chauffeur.php';
require_once __DIR__ . '/Class/Contact.php';
require_once __DIR__ . '/Class/Form.php';
require_once __DIR__ . '/Class/Hotel.php';
require_once __DIR__ . '/Class/Mail.php';
require_once __DIR__ . '/Class/navbar.php';
//require_once __DIR__ . '/Class/PHPMailer.php';
require_once __DIR__ . '/Class/Profil.php';
require_once __DIR__ . '/Class/Restaurant.php';
require_once __DIR__ . '/Class/Service.php';
require_once __DIR__ . '/Class/Session.php';
require_once __DIR__ . '/Class/Trajet.php';
?>
