...
/*
    ...
    */
    ...
    public static function getTrajet($bdd,$state,$state2,$state3) {
      $req = $bdd->getPDO()->prepare('SELECT * FROM trajet  WHERE state = :state OR state = :state2 OR state = :state3 HAVING idTrajet < (SELECT MAX(idTrajet) FROM trajet)');
      $req->execute(array('state' => $state,'state2' => $state2,'state3' => $state3));
      return $req;
      $req->closeCursor();
